var $ = require("jquery"),
  ...
  template = require("text-loader!../../templates/ntees/item.html"),
  details = require("text-loader!../../templates/ntees/details.html");
...
var NteeView = Backbone.View.extend({
  ...
  render: function() {
    ...
    $("title").text(
      this.model.get("name") + " grant data" + " - The Detroit Ledger"
    );

    $("#content").html(this.details({}));

    var organizations = new Organizations.Collection(
      { orgs: this.model.get("orgs") },
      {
        parse: true,
      }
    );
